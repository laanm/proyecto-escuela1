class javaverlistaespera{
    ...
    exportarexcel(){
        let html=`
            ...
            <tr>
                ...
                <th>Nombre madre</th>
                <th>Telefonos madre</th>
                <th>Correo madre</th>

                <th>Nombre padre</th>
                <th>Telefonos padre</th>
                <th>Correo padre</th>
            </tr>
            ...
            ${this.lista.map((dat,index)=>{
                ...
                <tr">
                    ...
                    <td>${dat.nombrealumno}</td>

                    <td>${dat.madrenombre}</td>
                    <td>${dat.madretelefono}</td>
                    <td>${dat.madrecorreo}</td>

                    <td>${dat.padrenombre}</td>
                    <td>${dat.padretelefono}</td>
                    <td>${dat.padrecorreo}</td>
                </tr>
                `
            }).join("")}
        ...
        //let wb= XLSX.utils.table_to_book(document.getElementById("Principal"));  //let wb= XLSX.utils.table_to_book(document.getElementById("TablaAsistencia"));
        XLSX.writeFile(wb,"RegistroPublico.xlsx");
    }
    cargarcursos(){
        $("#Cargando").show()
        $.ajax({
    ...
    cargarlista(){
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                verlistaespera.activo=data.activo
    ...
    mostrardatos(){
        ...
        let html=`
            ...
            ${this.lista.map((dat,index)=>{
                ...
                return`
                <tr id="F-${index}" onclick="verlistaespera.mostrarcontacto(${index})">
                    <td>${this.config+index}</td>
                    <td>${dat.fechaemision.slice(0,10)}</td>
                    <td>${dat.nombrecurso}</td>
                    ...
                    <td><button class="BotonAprobar" onclick="verlistaespera.abrirmini(${dat.id})">Aceptar</button><button class="BotonRechazar" onclick="verlistaespera.editar(${dat.id},0)">Rechazar</button><button>Editar</button></button></td>
                </tr>
                `
            }).join("")}
    ...
    mostrarcontacto(x){
        $(".Contacto").remove()
        $(".Contacto2").remove()
        $(".ExtensionContacto").removeClass("ExtensionContacto")
        $(`#F-${x}`).addClass("ExtensionContacto")
        $(`#F-${x}`).after(`


        <tr class="Contacto">
            <th>Contacto Madre</th>
            <td colspan="5">${this.lista[x].madrenombre} ${this.lista[x].madretelefono} ${this.lista[x].madrecorreo}</td>
           
        <tr>

        <tr class="Contacto2">
            <th>Contacto Padre</th>
            <td colspan="5">${this.lista[x].padrenombre} ${this.lista[x].padretelefono} ${this.lista[x].padrecorreo}</td>
        </tr>
    ...
    Activar(){
        ...
        if($("#Activo").is(":checked")){
            ...
            if(confirm("Se desactivara el registro publico")){
