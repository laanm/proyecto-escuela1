...
def resumen(request):
    if metabase.objects.filter(id=1).exists():
        datos_metabase = metabase.objects.get(id=1)
    else:
        datos_metabase = metabase.objects.create(id=1)
    return render(request, 'inventario/resumen.html',{'metabase': datos_metabase})
