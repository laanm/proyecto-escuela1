...
class resumen_data(APIView):
    def get(self, request):
        try:
            ...
            compras['enero'] = campo_activo.objects.filter(fecha_compra__year=year, fecha_compra__month=1).aggregate(Sum('valor_total'))

            compras['febrero'] = campo_activo.objects.filter(fecha_compra__year=year, fecha_compra__month=2).aggregate(Sum('valor_total'))

            compras['marzo'] = campo_activo.objects.filter(fecha_compra__year=year, fecha_compra__month=3).aggregate(Sum('valor_total'))

            compras['abril'] = campo_activo.objects.filter(fecha_compra__year=year, fecha_compra__month=4).aggregate(Sum('valor_total'))

            compras['mayo'] = campo_activo.objects.filter(fecha_compra__year=year, fecha_compra__month=5).aggregate(Sum('valor_total'))

            compras['junio'] = campo_activo.objects.filter(fecha_compra__year=year, fecha_compra__month=6).aggregate(Sum('valor_total'))

            compras['julio'] = campo_activo.objects.filter(fecha_compra__year=year, fecha_compra__month=7).aggregate(Sum('valor_total'))

            compras['agosto'] = campo_activo.objects.filter(fecha_compra__year=year, fecha_compra__month=8).aggregate(Sum('valor_total'))

            compras['septiembre'] = campo_activo.objects.filter(fecha_compra__year=year, fecha_compra__month=9).aggregate(Sum('valor_total'))

            compras['octubre'] = campo_activo.objects.filter(fecha_compra__year=year, fecha_compra__month=10).aggregate(Sum('valor_total'))

            compras['noviembre'] = campo_activo.objects.filter(fecha_compra__year=year, fecha_compra__month=11).aggregate(Sum('valor_total'))

            compras['diciembre'] = campo_activo.objects.filter(fecha_compra__year=year, fecha_compra__month=12).aggregate(Sum('valor_total'))
