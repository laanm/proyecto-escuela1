...
class asistencia_lista(APIView):
    def post(self, request):
        try:
            ...
            calendario=Calendario.objects.filter(ano=request.POST.get("year")).order_by("id")
            
            for x in calendario:
                if hoy.day==x.dia and hoy.month==x.mes and x.tipodia==1:
                    laborales+=1
                    break
            ...
            for x in registro:
                for i in cursos:
                    if x.cursodematricula.id==i.id:
                        ...
                        if final.get(i.id)==None:
                            temp=0
                        else:
                            temp=final[i.id]["acumulado"]
                            temp2=final[i.id]["cantidad"]
                        final[i.id]={"idcurso":i.id,"nombrecurso":i.nombre,"acumulado":x.diasasistidos1+x.diasasistidos2+x.diasasistidos3+x.diasasistidos4+temp,"cantidad":1+temp2}
